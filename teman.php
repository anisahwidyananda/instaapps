   ...
   <section>
        <div class="container">
            ...
            <div class="row justify-content-center">
                <div class="col-md-4">
                    <ul class="list-group list-group-flush">
                        <?php 
                              $dbc = new PDO ('mysql:host=localhost;dbname=instaapp','root','');
                              $flw = $dbc->prepare("SELECT users.nama,users.foto FROM teman, users WHERE teman.id_teman = users.id AND teman.status='mengikuti' and teman.id=:id");
			                  $flw->bindValue(':id', $_SESSION['id']);
			                  $flw->execute();
                              foreach ($flw as $flws) {
                        ?>
                         <li class="list-group-item">
                            <img src="gambar/<?= $flws['foto']; ?>" alt="amik" width="80" class="rounded-circle" />
                            <?= $flws['nama']; ?> 
                         </li>
                         <?php } ?>
                    </ul>
            ...
            <div class="row justify-content-center">
                <?php 
                    $dbc = new PDO ('mysql:host=localhost;dbname=instaapp','root','');
                    $teman = $dbc->prepare("SELECT nama,foto,id from users WHERE id!=:id AND id NOT IN (SELECT id_teman from teman where id=:id);");
                ...
                <div class="col-md-4">
                    <ul class="list-group list-group-flush">
                        ...
                        <li class="list-group-item"> 
                        <img src="gambar/<?= $tmn['foto']; ?>" alt="amik" width="80" class="rounded-circle" />
                        <?= $tmn['nama']; ?> 
